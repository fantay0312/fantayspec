...
本 skill 输出为视觉文件（PNG），不适用普通文档类输出规范。
...
```bash
node ~/.claude/skills/py-card/assets/capture.js <html> <png> <width> <height> [fullpage]
```

依赖：`~/.claude/skills/py-card/node_modules/` 中的 playwright。如报错：

```bash
cd ~/.claude/skills/py-card && npm install playwright && npx playwright install chromium
```

...
- 文件名格式：`{时间戳}--paper-{简短标题}__paper.md`
...
- arxiv URL → 调用 py-fetch 获取，或 WebFetch 获取摘要+正文
...
将三个原子的中间结果编织成一篇连贯的分析。按 `references/template.md` 的结构输出。
...
2. 读取 `references/template.md` 获取结构
3. 按模板填充综合结果
